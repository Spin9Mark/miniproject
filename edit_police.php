...
<body class="bg-light"> <!-- ใช้ bg-light เพื่อให้พื้นหลังเป็นสีเทาอ่อน -->
    ...
    <?php 
        ...
        $sql = "SELECT * FROM polices WHERE pl_id = '$plId' ";

        $result = mysqli_query($connection, $sql);

        $police = mysqli_fetch_array($result);
    ?>
            ...
            <div class="col-md-8 col-lg-7">
                ...
                <div class="card shadow-lg border-0 rounded-4"> <!-- เพิ่มเงา, เอาเส้นขอบออก, และทำให้มุมมนขึ้น -->
                    ...
                    <div class="card-header bg-primary text-white text-center py-3">
                        <h2 class="mb-0 fw-bold"><i class="bi bi-person-plus-fill me-2"></i>แก้ไขข้อมูลเจ้าหน้าที่</h2>
                    </div>

                    <div class="card-body p-4 p-md-5">
                        <form action="php/update_police.php" method="post">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control form-control-lg" name="pl_id" id="pl_id" placeholder="กรอกรหัสประจำตัว" value="<?= $police["pl_id"] ?>" required>
                            </div>

                            <div class="mb-4">
                                <label for="full_name" class="form-label fw-bold">ชื่อ-นามสกุล</label>
                                <input type="text" class="form-control form-control-lg" name="full_name" id="full_name" placeholder="เช่น นายสมชาย ใจดี" value="<?= $police["full_name"] ?>" required>
                            </div>

                            <div class="mb-4">
                                <label for="tel" class="form-label fw-bold">ยศ</label>
                                <input type="text" class="form-control form-control-lg" name="rank" id="rank" placeholder="กรอกยศของท่าน" value="<?= $police["rank"] ?>" required>
                            </div>

                            <div class="mb-4">
                                <label for="address" class="form-label fw-bold">เบอร์ติดต่อ</label>
                                <input class="form-control form-control-lg" name="tel" id="tel" rows="3" placeholder="กรอกเบอร์ที่สามารถติดต่อได้" value="<?= $police["tel"] ?>" required>
                            </div>
