  ...
  <div class="container-fluid mt-5">
    ...
    <div class="card shadow-lg">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h2 class="mb-0">Type of Model</h2>
        <a href="add_type.php" class="btn btn-success">
          <i class="bi bi-plus-circle-fill me-2"></i> Add Type
        </a>
      </div>
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-hover table-bordered align-middle text-center table-fixed-header">
            <thead>
              <tr>
                <th scope="col">ID</th>
                <th scope="col" class="text-start">Type Name</th>
              ...
              <?php
                if(mysqli_num_rows($result) > 0) {
                  foreach($result as $type){
                    ?>
                    <tr>
                      <th scope="row"><?= htmlspecialchars($type['type_id']) ?></th>
                      <td class="text-start"><?= htmlspecialchars($type['type_name']) ?></td>
                      <td>
                        <a href="php/delete_type.php?type_id=<?= htmlspecialchars($type['type_id']) ?>" class="btn btn-danger btn-sm btn-action-col">
                          <i class="bi bi-trash-fill"></i> Delete
                        </a>
                      </td>
                      <td>
                        <a href="edit_type.php?type_id=<?= htmlspecialchars($type['type_id']) ?>" class="btn btn-warning btn-sm btn-action-col">
                  ...
                  ?>
                  <tr>
                    <td colspan="4" class="text-center">No type data available.</td>
                  </tr>
                  <?php
                }
              ?>
